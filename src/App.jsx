...
import { ARTISTS, GALLERIES, GALLERY_AREAS, ART_CATEGORIES, ART_TIERS } from "./art";
        ...
        {selectedArtist && (
          <div style={S.modal} onClick={()=>setSelectedArtist(null)}>
            <div style={S.modalBox} onClick={e=>e.stopPropagation()}>
              ...
              <div style={S.mRank}>#{selectedArtist.rank} OF {ARTISTS.length}</div>
              <div style={S.mTitle}>{selectedArtist.name}</div>
              <div style={S.mArtist}>{selectedArtist.medium}</div>
              <div style={{...S.pills,marginBottom:12}}>
                <span style={{fontSize:8,padding:"3px 7px",borderRadius:99,background:signalColor(selectedArtist.signal).bg,color:signalColor(selectedArtist.signal).text,border:`1px solid ${signalColor(selectedArtist.signal).border}`,letterSpacing:"0.08em",textTransform:"uppercase"}}>{selectedArtist.signal}</span>
                <span style={{fontSize:8,padding:"3px 7px",borderRadius:99,background:"#1A1A1A",color:"#555",border:"1px solid #222",letterSpacing:"0.06em"}}>{selectedArtist.tier}</span>
                {selectedArtist.findable && <span style={S.findPill}>FINDABLE</span>}
              </div>
              <div style={S.mMeta}>
                <span style={S.mMetaH}>Medium:</span> {selectedArtist.medium}<br/>
                <span style={S.mMetaH}>Era:</span> {selectedArtist.era}<br/>
                <span style={S.mMetaH}>Category:</span> {selectedArtist.category}
              </div>
              <div style={S.mDesc}>{selectedArtist.description}</div>
              <div style={S.mValue}> Value range: {selectedArtist.value}</div>
              <div style={{marginTop:12}}><SaveBtn isFav={artistFavs.includes(selectedArtist.id)} onToggle={e=>toggle(setArtistFavs)(selectedArtist.id,e)} /></div>
            </div>
          </div>
        )}
